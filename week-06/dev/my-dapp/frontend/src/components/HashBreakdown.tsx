...
interface Props {
  ...
  choices: number[]       // 4 picks
  survivingMask: number   // bitmask (bit i = pick i survived minority game)
...
export function HashBreakdown({ revealHash, choices, survivingMask, eyeOrder, score }: Props) {
    ...
    <div className="bg-gray-900 rounded-xl p-5 border border-gray-800 space-y-4">
      ...
      <div>
        ...
        <div className="grid grid-cols-8 gap-1 font-mono text-xs">
          {nibbleMult.map((mult, v) => {
            ...
            const multLabel = `${(mult / 10).toFixed(1)}x`
            return (
              <div
                key={v}
      ...
      <div>
        ...
        <div className="grid grid-cols-4 gap-2">
          {choices.map((v, i) => {
            const survived = (survivingMask & (1 << i)) !== 0
      ...
      <div className="space-y-1.5">
        ...
        <div className="flex items-center justify-between bg-indigo-950 rounded-lg px-4 py-2.5 border border-indigo-800">
          ...
          <span className="text-white font-bold text-xl">{score.toString()}pt</span>
        </div>
      </div>
