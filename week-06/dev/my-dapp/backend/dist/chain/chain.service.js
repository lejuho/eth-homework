...
const REGISTRY_ABI = (0, viem_1.parseAbi)([
...
let ChainService = ChainService_1 = class ChainService {
    ...
    onModuleInit() {
        ...
        if (regAddr)
            this.logger.log(`Registry: ${regAddr}`);
        else
            this.logger.warn('REGISTRY_ADDRESS not set — multi-room disabled');
    ...
    async getOverlappingNibbles(roundId) {
    ...
    async expireRound(roundId) { return this.write('expireRound', [roundId]); }
    ...
    async createRound() {
        ...
        for (const log of receipt.logs) {
            try {
                const decoded = (0, viem_1.decodeEventLog)({ abi: ABI, eventName: 'RoundCreated', topics: log.topics, data: log.data });
                const roundId = decoded.args.roundId;
                const regHash = await this.registerRound(roundId);
                if (regHash)
                    await this.waitForReceipt(regHash);
